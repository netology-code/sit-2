...
Итак, требуется составить таблицу, для этого используем тег `table`. В таблице должна быть строка заголовков `thead`, строка итогов `tfoot` и, конечно, основной контент таблицы — `tbody`. Формируем основу таблицы:
```html
<table>
  <thead></thead>
  <tbody></tbody>
